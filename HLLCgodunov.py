import numpy as np, HLLCflux

def HLLCgodunov(rho, u, p, gamma, limit, dx, Ccfl = 0.9):
    ...
    while time < limit:
        ...
        result = np.array([HLLCflux.HLLCflux(currentu[j], currentu[j+1], currentp[j], currentp[j+1],
                               currentrho[j], currentrho[j+1], gamma) for j in range(l)])
